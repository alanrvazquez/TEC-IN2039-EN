...
The letter abbreviations below the column names describe the type of each variable:

::: incremental
- `dbl` stands for doubles or real numbers. For continuous variables.

- `int` stands for integers. For discrete variables.

- `fctr` stands for factors, which R uses to represent categorical variables (nominal and ordinal) with fixed possible values.

- `lgl` stands for logical, vectors that contain only `TRUE` or `FALSE`. For categorical variables (nominal).
