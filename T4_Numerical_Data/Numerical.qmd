...
In python, we calculate the mean using the function `mean()`.
...
plt.figure(figsize=(10, 4))
sns.histplot(data=piston_data, x="cycle_time", bins=10, 
             edgecolor="black", color="pink")
plt.title("Histogram of Cycle Time")
plt.xlabel("Cycle Time (seconds)")
plt.ylabel("Frequency")
plt.figtext(0.15, 0.01, "Data from 50 pistons", ha="left", fontsize=10)
plt.show()
```
...
Learn more in that league [**https://towardsdatascience.com/why-1-5-in-iqr-method-of-outlier-detection-5d07fdc82097**](#0){.uri}
...
plt.figure(figsize=(5, 4))
sns.boxplot(y=piston_data["cycle_time"])
plt.show()
```
...
plt.figure(figsize=(5, 4))
sns.boxplot( x = piston_data["cycle_time"])
plt.show()
```
